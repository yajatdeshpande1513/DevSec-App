...
export default function Home() {
  ...
  return (
    <main className={`min-h-screen flex items-center justify-center p-6 transition-all duration-1000 font-mono ${isVulnerable ? 'bg-red-950' : isError ? 'bg-slate-900' : 'bg-slate-950'}`}>
      ...
      {/* Main Glass Card */}
      <div className={`relative z-10 w-full max-w-2xl backdrop-blur-md bg-white/5 border-2 rounded-3xl p-8 shadow-2xl transition-all ${isVulnerable ? 'border-red-500 shadow-red-500/20' : isError ? 'border-amber-500/50 shadow-amber-500/20' : 'border-emerald-500/50 shadow-emerald-500/20'}`}>
        ...
        <div className="flex flex-col items-center py-10">
          <div className={`w-24 h-24 rounded-full flex items-center justify-center border-2 mb-6 transition-all ${isVulnerable ? 'border-red-500 bg-red-500/10' : isError ? 'border-amber-500 bg-amber-500/10' : 'border-emerald-500 bg-emerald-500/10'}`}>
            <span className="text-4xl">{isVulnerable ? '⚠️' : isError ? '📡' : '🛡️'}</span>
          </div>
          <h1 className={`text-5xl font-black mb-2 text-center tracking-tighter ${isVulnerable || isError ? 'text-white' : 'text-transparent bg-clip-text bg-gradient-to-r from-emerald-400 to-cyan-400'}`}>
